...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_FullToolBar.CreateEx(this, TBSTYLE_FLAT,WS_CHILD| WS_VISIBLE|CBRS_TOP |CBRS_GRIPPER |   CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC) ||
     !m_FullToolBar.LoadToolBar(IDR_TOOLBAR1,0, 0, TRUE, 0, 0, IDB_BITMAP1))
    {
     TRACE0("Failed to create toolbar/n");
     return -1;      // fail to create
    }
	m_FullToolBar.EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_FullToolBar);
	//图层切换工具栏
	EnableAutoHidePanes(CBRS_ALIGN_ANY);
	DWORD style = WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI;
	if(!m_wndLayer.Create( _T("文档管理"),this,CRect(0, 0, 200, 200),true,0,style))
	{
      return FALSE;
    }
